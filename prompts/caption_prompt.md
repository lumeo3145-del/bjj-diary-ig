...
- 無料で使える機能: 日記（カレンダー）、メモ、打ち込みリスト、試合記録
- Plusプラン(月額)限定の機能: 統計・グラフ（練習日数・試合成績・入賞率）、タグ管理・フィルタリング、カラーテーマ
- アプリ名は日英どちらも「BJJ Diary」。「柔術日記」という旧名称は一切使わない

### 無料/有料の書き分け（重要）
- 「App Storeで無料配信中」はアプリ本体のダウンロードが無料という意味。機能が全部無料という意味ではない
- **統計・グラフ / タグ / カラーテーマ を主役にした投稿では、無料を強調するコピーを書かない**
  （例: 「無料で統計が見られる」はNG。統計は Plus 限定）
- 無料アピール（アカウント登録不要・すぐ書ける等）は、日記・メモ・打ち込みリスト・試合記録の回で使う
